...
fn fuzz_this_thing(prng_seed: u64, iterations: usize) {
    ...
    // Preallocate space to avoid frequent reallocations
    seed_data.reserve(iterations / 500 * 10);
    ...
    // Use buffered output to improve performance
    let stdout = std::io::stdout();
    let mut writer = BufWriter::new(stdout.lock());
    writer.write_all(&seed_data).expect("Failed to write mutated data to stdout");
}
